...
To optimize wi\textsuperscript{th} respect to a parameter, use good ol' calculus: Take a derivative, set equal to zero, and solve. Let's select $\beta_{jk'}$, the $(k=k')$\textsuperscript{th} entry of $\vec{\beta_j}$ as the parameter of interest.
...
$$\vec{w_C} = \frac{\sum_{i=1}^n \pmb{H}}{\vec{w_I} \cdot \pmb{H}} \sum_{k=1}^q \pmb{L}$$
