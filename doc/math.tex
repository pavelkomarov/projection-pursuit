...
where the "hat" \string^ distinguishes the answers predicted, as opposed known training answers.\newline
...
where $f_j(\pmb{X} \cdot \vec{\alpha_j})$ is actually an $n$-vector.
...
This can be vectorized to find all entries of $\vec{\beta_j}$ at once. Notice that $\vec{w_I}$ is an $n$-vector, that the residuals come from $\pmb{R}_j$, and that the sums iterate the length-$n$ dimensions of these objects:
